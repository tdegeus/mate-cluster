...
1. The information is read by one of the following commands::
   ...
   These commands return a list of jobs/nodes. Each entry is a dictionary.
   Each dictionary field is of the "Data" class. This class deals with 
   arithmetic operations, and operations to return a readable string. This
   class deals with any "None" entries that correspond to missing data.
...
* Introduced the "Data" class as Swiss army knife handle all information fields.
  ...
  deals with "None" fields that are encountered if the data was not read. [TdG]

Version 1.2 - May 2014

* Numerous changes to accommodate myqstat better [TdG]
* Removed "HostInfo": qstatLog should be changed accordingly [TdG]
...
**strcmp**
  ...
  Compare a humanly readable string (e.g. ">1").
  '''
...
def qstatSummary(jobs,nodes,key='owner'):
  ...
  for job in jobs:
    ...
    if field not in out:
      ...
      out[field]['normtime'] = Data(Time(0.0),'%s','Time') # normalized time for score
      out[field]['owner'   ] = str(job['owner'])
    # add fields
    for name in ['cputime','memused','host','walltime']:
      out[field][name] += job[name]
    # add normalized walltime
    out[field]['normtime'] += Time(float(job['walltime'])*float(int(job['host'])))
  ...
  for key in out:
    ...
    # add score
    try:
      walltime   = float(out[key]['normtime'])
      cputime    = float(out[key]['cputime' ])
      ncpu       = float(int(host))
      out[key]['score'] = Data(cputime/(walltime),'%4.2f','float')
    except:
      out[key]['score'] = Data()
