...
The first option disables all display options within ``MATLAB``, which automatically disables the ``MATLAB`` desktop. The ``-singleCompThread``  (Available since 2009a, for older versions use the ``maxNumCompThreads(1)``
command in your m-file.) disables automatic parallelization which is required on the cluster, otherwise ``MATLAB`` will use more CPUs than assigned to the job by the queuing system, which will overload the node.
